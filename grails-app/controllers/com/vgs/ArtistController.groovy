...
class ArtistController {
	...
	static navigation = [
		[group:'searchArtist', action:'search', title: 'Search Artist', order: 0]
	]
    static allowedMethods = [save: "POST", update: "POST", delete: "POST"]
 
	def search = {
		// go through to search.gsp	
    }
	
	def results = {
	def artists = Artist.findAll("from Artist a where upper(a.name) like :name", 
			[name:"%${params.name}%".toUpperCase()])
		[ artists : artists, term : params.name]
    }
